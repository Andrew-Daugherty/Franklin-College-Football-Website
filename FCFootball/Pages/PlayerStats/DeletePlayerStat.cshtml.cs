...
public class DeletePlayerStatModel : PageModel
{

	private readonly FCFootballContext FCFootballContext;
	public DeletePlayerStatModel(FCFootballContext SPC)
	{
		FCFootballContext = SPC;
	}

	private PlayerStat PlayerStat { get; set; }
	...
	public async Task<IActionResult> OnGetAsync(int intPlayerStatID)
	{
		...
		if (PlayerStat != null)
		{
			try
			{
				...
				FCFootballContext.PlayerStats.Remove(PlayerStat);
		...
		return RedirectToPage("MaintainPlayerStats");
